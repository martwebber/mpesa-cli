...
# Function to remove conflicting installations
remove_old_installations() {
    log_header "🧹 Checking for conflicting installations..."
    
    # Find all mpesa-cli binaries
    OLD_INSTALLATIONS=$(find /usr /home 2>/dev/null | grep -E "bin/mpesa-cli$" | grep -v "/tmp/" | grep -v "$(brew --prefix 2>/dev/null || echo /nonexistent)" || true)
    
    if [ -n "$OLD_INSTALLATIONS" ]; then
        log_warn "Found potentially conflicting installations:"
        echo "$OLD_INSTALLATIONS" | while read -r installation; do
            if [ -f "$installation" ]; then
                VERSION=$("$installation" --version 2>/dev/null | head -n1 | cut -d' ' -f3 || echo "unknown")
                log_warn "  - $installation (version: $VERSION)"
            fi
        done
        
        echo
        read -p "$(echo -e "${YELLOW}Remove old installations? [y/N]:${NC} ")" -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            echo "$OLD_INSTALLATIONS" | while read -r installation; do
                if [ -f "$installation" ] && [ "$installation" != "/usr/local/bin/mpesa-cli" ]; then
                    log_info "Removing $installation"
                    sudo rm -f "$installation" || log_warn "Failed to remove $installation"
                fi
            done
        fi
    else
...
main() {
    ...
    echo
    log_info "💡 To avoid version conflicts in the future, use this updater:"
    echo "  curl -fsSL https://raw.githubusercontent.com/martwebber/mpesa-cli/main/scripts/update-mpesa-cli.sh | bash"
}
